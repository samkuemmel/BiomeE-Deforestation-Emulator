...
BIOMEE_OUTPUT_NC = os.path.expanduser("~/workflowsREU/amazon006.nc")
...
scenarios = {
    2.7: "~/workflowsREU/cerrado027.nc",
    1.8: "~/workflowsREU/cerrado018.nc",
    0.95: "~/workflowsREU/cerrado0095.nc"
    }

all_scenario_dfs = []

# Prepare the forcing data frame with a matching alignment column
df_forcing_clean["merge_year"] = df_forcing_clean["time"]
...
for pct, nc_path in scenarios.items():
    ...
    scenario_df = scenario_df.drop(columns=["merge_year", "time_forcing", "time_sim"], errors="ignore")
...
#Save file for the next cell #this will be put into the master_df file path theremaster_training_df.to_csv(os.path.join(LOCAL_TMP, "crmaster_training_data.csv"), index=False)
